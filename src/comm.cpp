...
namespace Common{
	...
	_get_packet:
		...
		if (psdp->type == csdp_type::csdp_alloc || psdp->type == csdp_type::csdp_local){
			EtwLog("[写线程] 取得一个发送数据包, 长度为 %d 字节", psdp->cb);
			...
			for (nWrittenData = 0; nWrittenData < psdp->cb;){
				...
				if (bRet != FALSE){ // I/O is completed
					...
					if (bRet){
						EtwLog("[写线程] I/O completed immediately, bytes : %d", nWritten);
					}
					else{
	...
	_get_packet:
		...
		for (nTotalRead = 0; nTotalRead < nBytesToRead;){
			...
			if (bRet != FALSE){
				...
				if (bRet) {
					EtwLog("[读线程] 读取 %d 字节, bRet==TRUE, nBytesToRead: %d", nRead, nBytesToRead);
				}
				else{
				...
				if (::GetLastError() == ERROR_IO_PENDING){
					...
					case WAIT_OBJECT_0 + 1:
						bRet = ::GetOverlappedResult(_hComPort, &overlap, &nRead, FALSE);
						if (bRet){
							EtwLog("[读线程] 读取 %d 字节, bRet==FALSE", nRead);
						}
						else{
